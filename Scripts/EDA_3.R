...
copia_grupos <- data.table(Trade_valido[, c("CustomerIdx", "IsinIdx", "CustomerInterest", "dia_intervalo", "semana", "BuySell")])
...
semana_test <- 121
dia_test <- max(Trade_valido$dia_intervalo) + 3
#i <- 19
...
copia_grupos$Buy <- ifelse (copia_grupos$BuySell=='Buy', 1, 0)
copia_grupos$Sell <- ifelse (copia_grupos$BuySell=='Sell', 1, 0)
...
vdias_primeravez <- numeric(nrow(aux))
vdias_ultimavez <- numeric(nrow(aux))
vintereses_anteriores <- numeric(nrow(aux))
vNumBuy <- numeric(nrow(aux))
vNumSell <- numeric(nrow(aux))
vlastaction <- character(nrow(aux))
vlastBuy <- numeric(nrow(aux))
vlastSell <- numeric(nrow(aux))
#7min
gc()
system.time({
  for (i in 1:nrow(aux)){
    ...
    vdias_primeravez[i] <- dia_test - min(bbb$dia_intervalo[bbb$CustomerInterest==1])
    vdias_ultimavez[i] <- dia_test - max(bbb$dia_intervalo[bbb$CustomerInterest==1])
    vintereses_anteriores[i] <- sum(bbb$CustomerInterest[bbb$semana < semana_test])
    vNumBuy[i] <- sum(bbb$Buy[bbb$CustomerInterest==1 & bbb$semana < semana_test])
    vNumSell[i] <- sum(bbb$Sell[bbb$CustomerInterest==1 & bbb$semana < semana_test])
    vlastaction[i] <- bbb$BuySell[bbb$semana == max(bbb$semana[bbb$CustomerInterest==1 & bbb$semana < semana_test])]
    vlastBuy[i] <- semana_test - max(bbb$semana[bbb$CustomerInterest==1 & bbb$Buy==1 & bbb$semana < semana_test])
    vlastSell[i] <- semana_test - max(bbb$semana[bbb$CustomerInterest==1 & bbb$Sell==1 & bbb$semana < semana_test])
  }
}) 
...
aux$dias_primerinteres <- vdias_primeravez
aux$dias_ultimointeres <- vdias_ultimavez
aux$intereses_anteriores <- vintereses_anteriores
aux$numBuy <- vNumBuy
aux$numSell <- vNumSell
aux$lastaction <- vlastaction
aux$lastBuy <- vlastBuy
aux$lastSell <- vlastSell

summary(aux)

test <- sqldf ("SELECT c.*, a.num_interest1, a.num_interest2, a.num_interest3, a.num_interest4,
               a.num_interest5, a.num_interest6, a.num_interest12, a.num_interest27, a.dias_primerinteres,
               a.dias_ultimointeres, a.intereses_anteriores, a.numBuy, a.numSell, a.lastaction, a.lastBuy, a.lastSell
...
rm(copia_grupos, copia_test, aux, clientes_test, bonos_test, v1, v2, v3, v4, v5, v6, v12, v27, dia_test, bbb,
   vlastaction, vlastBuy, vlastSell, vNumBuy, vNumSell, semana_test, vdias_primeravez, vdias_ultimavez, vintereses_anteriores,
   Challenge)

#################################################
copia_Trade_valido <- Trade_valido[,c("CustomerIdx", "IsinIdx", "BuySell", "CustomerInterest", "semana",
                                    "fecha", "num_interest27", "TradeStatus", "TradeDateKey", "Price", "dia_intervalo")]
#Bucle principal:
...
Trade_valido$Buy <- ifelse (Trade_valido$BuySell == 'Buy', 1, 0)
Trade_valido$Sell <- ifelse (Trade_valido$BuySell == 'Sell', 1, 0)
...
vdias_primeravez <- numeric(nrow(unicos))
vdias_ultimavez <- numeric(nrow(unicos))
vintereses_anteriores <- numeric(nrow(unicos))
vNumBuy <- numeric(nrow(unicos))
vNumSell <- numeric(nrow(unicos))
vlastaction <- character(nrow(unicos))
vlastBuy <- numeric(nrow(unicos))
vlastSell <- numeric(nrow(unicos))
...
system.time(
  for (s in semanas_validas){
    ...
    dia <- min(Trade_valido$dia_intervalo[Trade_valido$semana == s])
    ##- Feature generation?
    for (i in 1:nrow(unicos)){
      ...
      vdias_primeravez[i] <- dia - min(bbb$dia_intervalo[bbb$CustomerInterest==1])
      vdias_ultimavez[i] <- dia - max(bbb$dia_intervalo[bbb$CustomerInterest==1])
      vintereses_anteriores[i] <- sum(bbb$CustomerInterest[bbb$semana < s])
      vNumBuy[i] <- sum(bbb$Buy[bbb$CustomerInterest==1 & bbb$semana < s])
      vNumSell[i] <- sum(bbb$Sell[bbb$CustomerInterest==1 & bbb$semana < s])
      vlastaction[i] <- ifelse(bbb$semana > s, 'vacio', 
                               bbb$BuySell[bbb$semana == max(bbb$semana[bbb$CustomerInterest==1 & bbb$semana < s])])
      vlastBuy[i] <- s - max(bbb$semana[bbb$CustomerInterest==1 & bbb$Buy==1 & bbb$semana < s])
      vlastSell[i] <- s - max(bbb$semana[bbb$CustomerInterest==1 & bbb$Sell==1 & bbb$semana < s])
    }
    ...
    unicos$dias_primerinteres <- vdias_primeravez
    unicos$dias_ultimointeres <- vdias_ultimavez
    unicos$intereses_anteriores <- vintereses_anteriores
    unicos$numBuy <- vNumBuy
    unicos$numSell <- vNumSell
    unicos$lastaction <- vlastaction
    unicos$lastBuy <- vlastBuy
    unicos$lastSell <- vlastSell
    ...
    esta_semana_completo <- sqldf ("SELECT t.*, a.num_interest1, a.num_interest2, a.num_interest3, a.num_interest4,
                                              ...
                                              a.dias_primerinteres, a.dias_ultimointeres, a.intereses_anteriores,
                                              a.numBuy, a.numSell, a.lastaction, a.lastBuy, a.lastSell
    ...
    esta_semana_completo$dia_intervalo <- ifelse(is.na(esta_semana_completo$dia_intervalo), min(esta_semana_grup$dia_intervalo), esta_semana_completo$dia_intervalo)
...
table(resul$BuySell)
resul[,c(7)] <- NULL
names(resul)[names(resul)=="num_interest27.1"] <- c("num_interest27")
...
write.csv(test,"C:/Users/Enric/Google Drive/Competiciones_MachineLearning/DSG_2018_2.0/Data/V3/test2.csv")
write.csv(resul,"C:/Users/Enric/Google Drive/Competiciones_MachineLearning/DSG_2018_2.0/Data/V3/Train_Generado3.csv")
